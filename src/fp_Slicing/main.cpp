...
int main()
{
    ...
    for (i=(npe.getRoot())->data.begin(); i!=(npe.getRoot())->data.end(); ++i)
        if (i->fisrt*i->second < min_area)
        {
            min_area = i->fisrt*i->second;
            t = i;
        }
...
void Stockmeyer(BinNode<Stockmeyer_Datatype> *node, myMap &block_size)
{
    ...
    Stockmeyer(node->lchild);
    Stockmeyer(node->rchild);

    Stockmeyer_Datatype::iterator i, j;
    if (node->flag == "H")
    {
        i = node->lchild->data->rbegin();
        j = node->rchild->data->rbegin();
    ...
    if (node->flag == "V")
    {
        i = node->lchild->data->begin();
        j = node->rchild->data->begin();
